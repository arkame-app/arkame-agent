...
**Status:** Sprints 5 (enrollment + bearer auth), 6 (sync engine + probe reporter) e Tier 1 C (restore execution) **concluídas**. Build limpo (`go vet ./...`).
...
- **`executePlan` (backup)**: POST `/sessions/start` → `sync.Run` (walker + hash + S3 PutObject + version_map) → POST `/sessions/{sid}/complete` com version_map inline; em falha total POST `/sessions/{sid}/fail`
- **`restore.Run` (restore)**: `internal/restore/executor.go` — GetObject(VersionId) → escrita atômica (tmp + rename) com SHA-256 verify → conflict resolution `suffix-version`/`overwrite`/`skip`; respeita `HOST_ROOT` (em container Docker = `/host`)
...
- **Multipart download** pra restores grandes (executor atual usa `GetObject` simples)
- **Dedup file-level**: checar HeadObject + sha256 metadata antes de re-upload
- **Warming cold storage**: detectar `STORAGE_CLASS=GLACIER`/`DEEP_ARCHIVE` no HeadObject e disparar RestoreObject antes de baixar; persistir `warming_state` em `restore_items`
